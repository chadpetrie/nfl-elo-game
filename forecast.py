...
# Resolved against this file rather than the working directory, so the model can be imported
...
class Forecast:
    ...
    def forecast(games):
        ...
        Also records each team's pre-game rating in my_elo1/my_elo2 and returns the final team
        ratings, so callers can show power rankings without replaying the model themselves.
        """

        # Initialize team objects to maintain ratings
        teams = {}
        with open(INITIAL_ELOS) as f:
        ...
        for game in games:
            ...
            # Record the ratings the forecast was actually made from
            game['my_elo1'], game['my_elo2'] = team1['elo'], team2['elo']
            ...
            if game['score1'] != None:
                ...
                mult = math.log(max(pd, 1) + 1.0) * (MOV_BASE / (1.0 if game['result1'] == 0.5 else ((elo_diff if game['result1'] == 1.0 else -elo_diff) * 0.001 + MOV_BASE)))
